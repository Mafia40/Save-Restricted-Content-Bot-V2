...
async def chk_user(message, user_id):
    ...
    if user_id in user or user_id in OWNER_ID:
        ...
        await message.reply_text("Contact @Akshat_Jaiin to Purchase premium :-)")
        return 1
...
async def subscribe(app, message):
   ...
   if update_channel:
      try:
         ...
         await message.reply_photo(photo="https://i.postimg.cc/NF0cDMQM/welcome.jpg",caption=script.FORCE_MSG.format(message.from_user.mention), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Join Now...", url=f"{url}")]]))
         return 1
      except Exception:
